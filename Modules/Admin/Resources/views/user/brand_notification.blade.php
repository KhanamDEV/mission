                ...
                <table class="table wrap-table">
                    ...
                    @foreach($data['notifies'] as $notify)
                        <tr>
                            <td class="box-name">
                                <div class="name ml-0">
                                    <a href="{{route('admin.user_brand_notification', ['user_id' => request()->route('user_id'),'id' => $notify->id])}}" title="">{{$notify->title}}</a>
                                    <p class="text-less">{{date('Y/m/d', strtotime($notify->created_at))}}</p>
                                </div>
                            </td>
                            <td>
                                <a href="{{route('admin.user_brand_notification', ['user_id' => request()->route('user_id'),'id' => $notify->id])}}" class="control-arrow"><img src="{{asset('static/admin/images/icon-arrow.png')}}" alt=""></a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
