    ...
    <section id="wrap-misson-detail">
        <div class="detail-base bg-white pt-0">
            <div class="detail-title-misson text-center">
                <h3 class="font-bold">@lang('manager::layer.notification.detail')</h3>
            </div>
            <div class="border"></div>
            <div class="line"></div>
            <div class="box-text">
                <p>@lang('manager::layer.notification.title'): <span class="font-bold">{{$data['notify']->title ?? ''}}</span> </p>
                <p>@lang('manager::layer.notification.description') :  <span>{{$data['notify']->description ?? ''}}</span> </p>
                <p>@lang('manager::layer.notification.delivery_destination') : <span class="font-bold">{{\App\Helpers\Helpers::renderDestinationNotification($data['notify']->type)}}</span> </p>
                @if(!is_null($data['notify']->option))
                    ...
                    @endif
                @endif
                <p>@lang('manager::layer.notification.url') : <span class="font-bold">{{$data['notify']->url}}</span> </p>
            </div>
            <div class="line"></div>
            <div class="wrap-button text-center d-flex align-items-center justify-content-center flex-column">
                <a href="{{route('manager.notification_edit', ['brand_id' => request()->route('brand_id'), 'id' => request()->route('id')])}}" class="btn button-border button-edit" title="">@lang('manager::layer.notification.btn_edit')</a>
            </div>
            <div class="line"></div>
            <div class="line"></div>
            <div class="wrap-button text-center d-flex align-items-center justify-content-center flex-column">
    ...
    <script>
        $(function (){
            $(".button-delete").click(function (){
                loading(1);
                $.ajax({
                    url: '{{route('manager.notification_delete', ['brand_id' => request()->route('brand_id'), 'id' => request()->route('id')])}}',
                    type: 'POST',
                    data: {_token: '{{@csrf_token()}}', id: {{request()->route('id')}}},
                    success: function (res){
                        loading()
                        if (res.meta.status == 200){
                            window.location.replace('{{route('manager.notification_index', ['brand_id' => request()->route('brand_id')])}}')
                        } else{
