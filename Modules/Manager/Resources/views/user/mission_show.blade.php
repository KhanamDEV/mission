...
@section('content')
    <section id="wrap-user">
        ...
        <div class="wrap-list-user text-center detail-select">
            <div class="text-left">
                <a href="{{route('manager.user_show', ['brand_id' => request()->route('brand_id'), 'id' => $data['user']->id ])}}" title="" class="btn auto-width button-border back-prev">
            ...
            <img src="{{empty($data['mission']->thumbnail_url) ? asset('static/manager/images/detail-selected.png') : \App\Helpers\Helpers::getUrlImg($data['mission']->thumbnail_url)}}" alt="" class="img_detail">
                ...
                <table class="table wrap-table misson-table">
                    ...
                    @foreach($data['questions'] as $key => $question)
                        <tr>
                            <td class="box-name">
                                <div class="name">
                                    <p class="title-question font-bold">Q.{{$key + 1}} : {{$question->title}}</p>
