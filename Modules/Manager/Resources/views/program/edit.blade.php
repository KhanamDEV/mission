...
@section('content')
    <section id="brand-new">
        ...
        <form method="POST" id="form-edit-program" enctype="multipart/form-data" class="wrap-form">
            @csrf
            <div class="box-main-new bg-white text-center">
                <div class="form-group drop-file relative">
                    ...
                    <div style="background: #eef0f8 url('{{\App\Helpers\Helpers::getUrlImg($data['program']->thumbnail_url)}}') no-repeat top center; background-size: contain; display: block; background-position: center" class="preview"></div>
                </div>
                <button type="" class="btn btn-color-main upload-image ">@lang('manager::layer.program.upload_image')</button>
            </div>
            <div class="box-main-new bg-white no-border">
                <div class="form-group">
                    <label for="">@lang('manager::layer.program.title')</label>
                    <input type="text" value="{{$data['program']->name}}" name="name" id="" class="form-control">
                </div>
                <div class="form-group">
                    <label for="">@lang('manager::layer.program.detail')</label>
                    <textarea name="detail" id="" class="form-control">{{$data['program']->detail}}</textarea>
                </div>
                <div class="form-group">
                    <label for=""></label>
                    <select name="status" id="" class="form-control">
                        <option @if($data['program']->status) selected @endif value="1">@lang('layer.active')</option>
                        <option @if(!$data['program']->status) selected @endif value="0">@lang('layer.not_active')</option>
                    </select>
                </div>
            </div>
            <p class="text-center title-brand">@lang('manager::layer.program.mission') <a href="#"
                                                                                          class="btn auto-width btn-white"
                                                                                          data-toggle="modal"
                                                                                          data-target="#new-program">@lang('manager::layer.program.new')</a>
            </p>
            <div class="list-file list-program bg-white">
                <table class="table wrap-table">
                    ...
                    @foreach($data['program_mission'] as $programMission)
                        <tr>
                            <td>{{$programMission->name}}</td>
                            <td class="input-date">
                                {{$programMission->delivery_date_number}}
                                <span>@lang('manager::layer.program.what_day')</span>
                                    <input type="hidden" name="mission[{{$programMission->mission_id}}]"
                                           value="{{$programMission->delivery_date_number}}"
                                           class="form-control sub-mission">
                            </td>
                            <td class="text-right">
                                <button value="{{$programMission->id}}"
                                        class="btn button-border auto-width remove-mission" type="button">
            ...
            <div class="wrap-button text-center d-flex align-items-center justify-content-center flex-column">
                <button type="submit" onclick="disabledForm('form-edit-program')" id="submit-form" class="btn bg-blue" title="">保存</button>
                <a href="{{route('manager.program_show', ['id' => request()->route('id')])}}" class="btn button-border button-edit" title="">保存しないで戻る</a>
            </div>
        </form>
    </section>
@endsection
...
@section('validate')
    ...
    {!! JsValidator::formRequest('Modules\Manager\Http\Requests\EditProgramRequest', '#form-edit-program') !!}
@endsection
@section('script')
    <script>
        $(function () {
            ...
            let arrIdMission = [];
            @foreach($data['program_mission'] as $programMission)
            arrIdMission.push({{$programMission->mission_id}});
            @endforeach
            let mission;
            let day;
                            ...
                            $.ajax({
                                ...
                                success: function (res) {
                                    ...
                                    if (res.response.length == 0) {
                                        $("#add-mission").attr('disabled', 'disabled');
                                        modal.find("input[name='mission_id']").addClass("is-invalid");
                                        modal.find('span#mission_id-error').text('クエストは存在しません');
