...
@section('content')
    <section id="wrap-user-new">
        <form method="POST" id="update-user" class="wrap-form" enctype="multipart/form-data">
            @csrf
                ...
                <div class="form-group mb-0">
                    <div class="form-group drop-file relative mb-0">
                        <div class="active after drop-image">
                            ...
                            <div class="b-drop">
                                {!! __('user::layer.user.input_import') !!}
                            </div>
                        </div>
                        <div class="fill"></div>
            ...
            <div class="wrap-button-new text-center">
                <button type="button" id="submit-form" class="btn bg-blue">@lang('user::layer.user.button_upload')</button>
            </div>
        </form>
        <div class="list-file bg-white">
            <table class="table wrap-table">
                ...
                <tbody>
                @foreach($data['log_update'] as $logUpdateUser)
                    <tr>
                        <td>{{\App\Helpers\Helpers::formatDateTime($logUpdateUser->created_at, 'Y/m/d/H:i')}}</td>
                        <td>{{$logUpdateUser->file_name}}</td>
                        <td class="text-right"><a href="{{\App\Helpers\Helpers::getUrlImg($logUpdateUser->file_url)}}" title="" class="control-arrow down-file">@lang('manager::layer.user.download')</a></td>
                    </tr>
                @endforeach
...
@endsection
@section('validate')
    {!! JsValidator::formRequest('Modules\User\Http\Requests\UpdateUserRequest', '#update-user') !!}
@endsection
@section('script')
