...
use App\Service\User\Auth\PasswordService;
use Illuminate\Contracts\Support\Renderable;
use Illuminate\Http\Request;
use Illuminate\Routing\Controller;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\MessageBag;
use Modules\User\Http\Requests\ConfirmMailForgetPasswordRequest;
...
class PasswordController extends Controller
{
    ...
    public function confirmMail()
    {
        try {
            ...
            return view('user::auth.password.confirm_mail', compact('data'));
        } catch (\Exception $e) {
            abort('500');
        }
    }
    ...
    public function postConfirmMail(ConfirmMailForgetPasswordRequest $request)
    {
        try {
            ...
            if ($token) {
                ...
                return redirect()->route('user.forget_password_confirm');
            }
    ...
    public function confirm()
    {
        try {
            ...
            $data['common'] = Helpers::titleAction([__('layer.title_mission')]);
            return view('user::auth.password.confirm', compact('data'));
        } catch (\Exception $e) {
            abort('500');
        }
    }
    ...
    public function change($token)
    {
        try {
            ...
            $data['common'] = Helpers::titleAction([__('layer.title_mission')]);
            return view('user::auth.password.change', compact('data'));
        } catch (\Exception $e) {
            abort('500');
        }
    }
